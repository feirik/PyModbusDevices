...
# TODO - Pressure, automatic control
...
# Ad 202: MIXER - On/off turning on mixer in mixing tank required for processing the components
...
MID_HEATER_TEMP = 90 # Mid-point reference (temperature at 50% heater setting)
...
class ModbusServer:
    ...
    def simulate_data(self):
        while True:
            if self.coils[AUTO_CONTROL_ENABLE] == 1:
                ...
                process_sequence_thread = threading.Thread(target=self.process_sequence_simulation)
    ...
    def update_inlet_valve_positions(self):
        ...
        if self.powder_inlet_prop < target_prop_powder:
            ...
            self.owder_inlet_prop -= PROPORTIONAL_VALVE_INCREMENT
            self.powder_inlet_prop = max(self.inlet_powder_prop, target_prop_powder)
    ...
    def process_chemicals(self):
        ...
        if mixer_on == 1 and current_temp >= MIN_REACTION_TEMP:
            ...
            # Debugging
            if self.debug:
                print(f"Debug - Chemical Processing | Mixer: {'On' if mixer_on else 'Off'}, Temp: {current_temp}°C, Rate: {conversion_rate}, Processed Powder: {processed_powder}, Processed Liquid: {processed_liquid}, Product Level: {self.holding_registers[PROCESSED_PRODUCT_LEVEL_ADDR]}")
